...
for (i in 1:nrow(BTC_1h_price2)) {
  ...
  if (BTC_1h_price2[i,1] == btc1[k,1]) {
    k=k+1
    buy_position <- buy_position + 1
    buyin <- BTC_1h_price2[i,"Open"] %>% as.numeric()
    tem_atr <- BTC_1h_price2[i,"ATR"] %>% as.numeric()
    stop_loss <- buyin -  tem_atr
    stop_profit <- buyin +  tem_atr
  }
  if(BTC_1h_price2[i,"Low"] < stop_loss){ #最低價小過止損, 表示止損在價格區間內, 所以賠錢平倉
    if(buy_position > 0){
  ...
  if(BTC_1h_price2[i,"High"] > stop_profit){
    if (buy_position > 0) {
      tem_profit <- stop_profit - buyin #最高價大過止盈, 表示止盈在價格區間內, 所以賺錢平倉
      buy_position <- buy_position - 1
...
library(dplyr)
every_profit <- diff(data_trading1[,4]) %>% as.data.frame()
