...
#BTC_data1 <- read.csv("Binance_BTCUSDT_minute.csv",  sep = ",", stringsAsFactors = F, encoding = "UTF-8", header = F)
#BTC_data2 <- read.csv("Binance_BTCUSDT_2023_minute.csv",  sep = ",", stringsAsFactors = F, encoding = "UTF-8", header = F)
...
#BTC_1h_price2$Date <- as.POSIXct(BTC_1h_price2$Date)
#BTC_1h_price2 <- as.xts(BTC_1h_price2, order.by = BTC_1h_price2$Date)
...
data_trading <-  NULL
BTC_1h_2023 <- BTC_1h_price2[46929:nrow(BTC_1h_price2),]
BTC_1h_price2 <- BTC_1h_2023
...
btc1 <- BTC_1h_price2[BTC_1h_price2$Divergence == 1,] ##尋找出現訊號為1的資料
...
for (i in 1:(nrow(BTC_1h_price2))) {
  ...
  tem1 <- cbind(BTC_1h_price2[i,c("Date", "High", "Low","Close", "ATR", "Divergence")], buy_profit, stop_profit, stop_loss)
  data_trading <- rbind(data_trading, tem1)
  rm(tem_atr)
}

data_buy_profit1 <- data_trading[!duplicated(data_trading$buy_profit),]

library(dplyr)
every_profit <- diff(data_buy_profit1[,6]) %>% as.data.frame() #6是buy profit
every_profit <- rbind(NA,every_profit)
colnames(every_profit) <- "profit"

data_buy_profit1 <- cbind(data_buy_profit1, every_profit) ##end

data2017 <- subset(data_buy_profit1, grepl("2017", data_buy_profit1$Date))
data2018 <- subset(data_buy_profit1, grepl("2018", data_buy_profit1$Date))
data2019 <- subset(data_buy_profit1, grepl("2019", data_buy_profit1$Date))
data2020 <- subset(data_buy_profit1, grepl("2020", data_buy_profit1$Date))
data2021 <- subset(data_buy_profit1, grepl("2021", data_buy_profit1$Date))
data2022 <- subset(data_buy_profit1, grepl("2022", data_buy_profit1$Date))
data2023 <- subset(data_buy_profit1, grepl("2023", data_buy_profit1$Date))
...
for (i in 1:7) {
  n <- 2016+i
  tem <- subset(data_buy_profit1, grepl(n, data_buy_profit1$Date))
...
btcN1 <- BTC_1h_price2[BTC_1h_price2$Divergence == -1,]
...
for (i in 1:(nrow(BTC_1h_price2))) {
  ...
  if(BTC_1h_price2[i,"Divergence"] == -1){
    ...
    sell <- BTC_1h_price2[i,"Close"] %>% as.numeric()#sell是做空部位
    tem_atr <- BTC_1h_price2[i,"ATR"]%>% as.numeric()
    tematr1 <- rbind(tematr1, tem_atr)
    sell_trade_count <- sell_trade_count + 1
    stop_loss <- sell + tematr1[sell_trade_count]
    stop_profit <- sell - tematr1[sell_trade_count]
    if(BTC_1h_price2[i+1,"Low"] < stop_profit){ #最低價小過止損, 表示止損在價格區間內
      if(sell_position > 0){
        tem_profit <- sell - stop_profit
        sell_position <- sell_position - 1
      }
    }  
    if(BTC_1h_price2[i+1,"High"] > stop_loss){
      if (sell_position > 0) {
    ...
    tem1 <- cbind(BTC_1h_price2[i,c("Date", "High", "Low", "Close", "ATR", "Divergence")], sell_profit, stop_profit, stop_loss)
...
data_sell_profit1 <- data_sell_profit[!duplicated(data_sell_profit$sell_profit),]

every_sell_profit <- diff(data_sell_profit1[,"sell_profit"]) %>% as.data.frame()
every_sell_profit <- rbind(NA, every_sell_profit)
colnames(every_sell_profit) <- "sell profit"
data_sell_profit1 <- cbind(data_sell_profit1, every_sell_profit)
...
buy_drawdown <- min(data_buy_profit1$buy_profit)
buy_maxprofit <- max(data_buy_profit1$buy_profit)
sell_drawdown <- min(data_sell_profit1$sell_profit)
sell_maxprofit <- max(data_sell_profit1$sell_profit)
...
data_trade <- cbind(buy_win_percent, sell_win_percent, buy_profit, sell_profit, buy_drawdown, buy_maxprofit, sell_drawdown, sell_maxprofit)
buy_win_percent
sell_win_percent
buy_drawdown
buy_maxprofit
